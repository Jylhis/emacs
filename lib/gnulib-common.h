/* Common preprocessor block frozen from gnulib.
...
/* This header is the frozen concatenation of the AH_VERBATIM blocks
   that gnulib's autoconf macros gnulib-common.m4 and extern-inline.m4
   used to contribute to config.h in the autotools build.  Emacs no
   longer ships autotools (phase-10 cutover, commit 313f867), so the
   content is checked in directly rather than re-extracted on every
   Meson configure run.

   To regenerate after an upstream gnulib bump:
     1. Fetch gnulib-common.m4 and extern-inline.m4 from
        https://git.savannah.gnu.org/cgit/gnulib.git/tree/m4/
     2. Re-run the AH_VERBATIM extraction logic preserved in git
        history at meson/extract_ah_verbatim.py before its deletion.
     3. Replace the body of this file (everything between the header
        guards) with the new extraction; keep this comment block. */

#ifndef EMACS_GNULIB_COMMON_H
#define EMACS_GNULIB_COMMON_H 1

/* === from gnulib-common.m4 === */
